...
internal class PermissionOrRoleHandler : AuthorizationHandler<PermissionOrRoleRequirement>
{
		...
		PermissionOrRoleRequirement requirement)
	{
		...
		if(requirement.RequiredRoles.Count != 0) {
			var _roles = context.User.Claims
				.Where(c => c.Type == "http://schemas.microsoft.com/ws/2008/06/identity/claims/role")
				.Select(c => c.Value);
			if(_roles.Any(r => requirement.RequiredRoles.Contains(r))) {
				context.Succeed(requirement);
				return Task.CompletedTask;
			}
		}
		if(requirement.RequiredPermissions.Count != 0) {
			var _permissions = context.User.Claims
				.Where(c => c.Type == "permissions")
				.Select(c => c.Value);
			if(_permissions.Any(p => requirement.RequiredPermissions.Contains(p))) {
				context.Succeed(requirement);
				return Task.CompletedTask;
			}
		}
